package com.rt.common.view.datapicker;
...
public class CustomDatePicker implements View.OnClickListener, PickerView.OnSelectListener {
    ...
    public CustomDatePicker(Context context, Callback callback, String beginDateStr, String endDateStr) {
        this(context, callback, TimeUtils.string2Millis(beginDateStr, "yyyy-MM-dd HH:mm"),
                TimeUtils.string2Millis(endDateStr, "yyyy-MM-dd HH:mm"));
    }
    ...
    private void initView() {
        mPickerDialog = new Dialog(mContext, com.rt.base.R.style.CommonBottomDialogStyle);
        ...
        ((TextView) mPickerDialog.findViewById(R.id.tv_cancel)).setText("取消");
        ((TextView) mPickerDialog.findViewById(R.id.tv_confirm)).setText("确定");
        ((TextView) mPickerDialog.findViewById(R.id.tv_reset)).setText("重置");
        ...
        mPickerDialog.findViewById(R.id.tv_reset).setOnClickListener(this);
    ...
    public void setResetVisibility(Boolean visibility) {
        ((TextView) mPickerDialog.findViewById(R.id.tv_reset)).setVisibility(View.GONE);
    }
    ...
    public boolean setSelectedTime(String dateStr, boolean showAnim) {
        return canShow() && !TextUtils.isEmpty(dateStr)
                && setSelectedTime(TimeUtils.string2Millis(dateStr, format), showAnim);
    }
    ...
    public void showTimeFormat(int type) {
        if (!canShow()) return;
        if (type == 1) {
     ...
     * 设置日期控件只显示年月
     */
    public void setShowYearMonth(boolean canShow) {
        if (!canShow()) return;
